...
public class QLearnerBothHands {
    private static final double LEARNING_RATE = 0.1;
    private static final double DISCOUNT_FACTOR = 0.9;
    private static final int NUM_EPISODES = 1000000;

    //dealer_hand_sum->own_hand_sum->action->value
    private static HashMap<Integer, HashMap< Integer, HashMap<Integer, Double>>> QTable;
    ...
    private static int selectAction(ArrayList<String> state, Random rand) {
        ...
        if (    !QTable.containsKey(current_dealer_hand_value) ||
                !QTable.get(current_dealer_hand_value).containsKey(current_own_hand_value) ||
            ...
            // Greedy action
            int bestAction = 0;
            double bestQValue = QTable.get(current_dealer_hand_value).get(current_own_hand_value)
                    .getOrDefault(0, 0.0); // Default to 0.0 if Q-value is not present
            for (int action : QTable.get(current_dealer_hand_value).get(current_own_hand_value).keySet()) {
                double qValue = QTable.get(current_dealer_hand_value).get(current_own_hand_value)
    ...
    private static void updateQValue(ArrayList<String> state, int action, ArrayList<String> nextState, double reward) {
        ...
        QTable.putIfAbsent(current_dealer_hand_value, new HashMap<>());
        QTable.get(current_dealer_hand_value).putIfAbsent(current_own_hand_value, new HashMap<>());
        QTable.get(current_dealer_hand_value).get(current_own_hand_value).putIfAbsent(action, 0.0);

        double currentQValue = QTable.get(current_dealer_hand_value).get(current_own_hand_value).get(action);
        ...
        QTable.get(current_dealer_hand_value).get(current_own_hand_value).put(action, newQValue);
    }
