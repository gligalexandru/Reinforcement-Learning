...
public class QLearnerOwnHandHoldAce {
    private static final double LEARNING_RATE = 0.1;
    private static final double DISCOUNT_FACTOR = 0.9;
    private static final int NUM_EPISODES = 1000000;

    //contains_ace->hand_sum->action->value
    private static HashMap<Boolean, HashMap<Integer, HashMap<Integer, Double>>> QTable;
    ...
    private static int selectAction(ArrayList<String> state, Random rand) {
        ...
        Boolean holds_active_ace = BlackJackEnv.holdActiveAce(BlackJackEnv.getPlayerCards(state));
        Integer current_hand_value = BlackJackEnv.totalValue(BlackJackEnv.getPlayerCards(state));

        if (!QTable.containsKey(holds_active_ace) ||
                !QTable.get(holds_active_ace).containsKey(current_hand_value) ||
            ...
            // Greedy action
            int bestAction = 0;
            double bestQValue = QTable.get(holds_active_ace)
                    .get(current_hand_value)
                    .getOrDefault(0, 0.0); // Default to 0.0 if Q-value is not present
            for (int action : QTable.get(holds_active_ace).get(current_hand_value).keySet()) {
                double qValue = QTable.get(holds_active_ace)
                        .get(current_hand_value)
    ...
    private static void updateQValue(ArrayList<String> state, int action, ArrayList<String> nextState, double reward) {
        ...
        QTable.putIfAbsent(holds_active_ace, new HashMap<>());
        QTable.get(holds_active_ace).putIfAbsent(current_hand_value, new HashMap<>());
        QTable.get(holds_active_ace).get(current_hand_value).putIfAbsent(action, 0.0);

        double currentQValue = QTable.get(holds_active_ace).get(current_hand_value).get(action);
        ...
        if (!Boolean.parseBoolean(nextState.get(0))) {
            ...
            maxNextQValue = QTable.get(holds_active_ace)
        ...
        QTable.get(holds_active_ace).get(current_hand_value).put(action, newQValue);
    }
